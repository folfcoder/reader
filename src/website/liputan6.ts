...
export async function parseLiputan6(url: string): Promise<newsData> {
    ...
    content = content.replace(/<div[^>]*class="advertisement-text".*?<\/div>/gis, "");

    // Remove page separator
    content = content.replace(/<div[^>]*class="article-content-body__item-break-index".*?<\/div><hr.*?>/gis, "");

    // Fix images
    content = content.replaceAll(" src=", " old-src=");
    content = content.replaceAll(" data-src=", " src=");
    content = content.replaceAll("<span class=\"read-page--photo-gallery--item__icon--new-zoom__enlarge-caption\">Perbesar</span>", "");
